...
const App: React.FC = () => {
  ...
  const DealsSection = () => (
    <section className="txa-deals-banner py-6 lg:py-8 rounded-[1.5rem] mx-0 lg:mx-0 overflow-visible shadow-2xl relative border border-[#e31b23]/40 mb-4 animate-in slide-in-from-right duration-500">
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 relative z-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
             <div className="flex items-center gap-3 flex-wrap mb-2">
               <h2 className="txa-deals-banner-title font-condensed text-3xl lg:text-5xl font-black uppercase tracking-tight italic leading-none drop-shadow-md">Offerte del Giorno</h2>
               <span className="txa-deals-banner-cta bg-[#e31b23] px-3 py-1 rounded text-xs font-black uppercase tracking-widest shadow-md">HOT</span>
             </div>
             <p className="txa-deals-banner-sub text-sm font-medium">Selezionate in tempo reale dal nostro canale Telegram.</p>
          </div>

          <a href="https://t.me/tuttoxandroid" target="_blank" rel="noopener noreferrer" className="txa-deals-telegram-pill flex items-center gap-3 bg-[#24A1DE] pl-2 pr-6 py-2 rounded-full transition-all group shadow-xl shrink-0 hover:scale-105 cursor-pointer">
              ...
              <div className="flex flex-col leading-none">
                <span className="txa-deals-banner-sub text-[10px] font-black uppercase mb-0.5">Canale Ufficiale</span>
                <span className="txa-deals-banner-title text-sm font-black uppercase tracking-wide">Offerte Italy</span>
              </div>
              <svg className="w-5 h-5 ml-2 text-white group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
          </a>
        </div>
        ...
        <div className={`grid gap-4 ${dealsToShow > 4 ? 'grid-cols-2 md:grid-cols-3 xl:grid-cols-4' : 'grid-cols-2 lg:grid-cols-4'} items-stretch`}>
          {deals.slice(0, dealsToShow).map(deal => (
            <a key={deal.id} href={deal.link} target="_blank" rel="noopener noreferrer" onClick={() => handleDealClick(deal, 'home_deals')} className="txa-deals-card bg-white rounded-xl shadow-md hover:shadow-xl transition-all group flex flex-col gap-3 p-4 hover:-translate-y-0.5 duration-300 min-h-[180px] border border-gray-100 relative">
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between">
                <h4 className="txa-deals-card-title font-bold text-sm mb-2 leading-snug line-clamp-3 transition-colors">{deal.product}</h4>
                <div className="flex items-baseline gap-2 flex-wrap">
                    <span className="txa-deals-card-price text-lg font-black tracking-tight">{deal.newPrice}</span>
