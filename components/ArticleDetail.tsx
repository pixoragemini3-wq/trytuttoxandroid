...
const ArticleDetail: React.FC<ArticleDetailProps> = ({ article, relatedArticle, moreArticles = [], deals = [], offerNews = [], onArticleClick }) => {
  ...
  // Logic to extract the "Most Significant" part for the quote block
  const highlightText = useMemo(() => {
    // Se non c'è contenuto, restituisci l'excerpt (che è già pulito dal service)
    ...
    try {
      ...
      for (const p of paragraphs) {
        ...
        // Count words inside bold tags
        boldTags.forEach(tag => {
          const text = tag.textContent || '';
          const words = text.trim().split(/\s+/).filter(w => w.length > 0);
          boldWordCount += words.length;
        });

        // If specific condition is met (> 6 bold words), return this paragraph text content
        // textContent decodes entities like &nbsp; automatically
        if (boldWordCount > 6) {
          return p.textContent || article.excerpt;
        }
      }
    } catch (e) {
      console.warn("Error parsing content for highlight", e);
    }

    // Fallback if no significant bold text is found
    // article.excerpt comes from bloggerService where it is now stripped of HTML entities
  ...
  return (
    <div className="bg-white border-b-8 border-gray-100 last:border-0 pb-12 mb-0 relative">
      ...
      <div className="max-w-3xl mx-auto px-4 pt-6 md:pt-16">
        ...
        {/* Title */}
        <h1 className="text-2xl md:text-5xl lg:text-6xl font-black text-gray-900 mb-6 md:mb-8 leading-tight md:leading-[0.95] tracking-tight">
          {article.title}
        </h1>
        ...
        {/* Content Body */}
        <div className="prose prose-base md:prose-xl max-w-none font-medium leading-relaxed text-gray-800 space-y-6">
          <p className="font-serif text-lg md:text-2xl leading-relaxed text-gray-600 border-l-4 border-[#e31b23] pl-6 italic mb-8">
            {highlightText}
          </p>
