...
const Step5Summary: React.FC<Step5Props> = ({ state }) => {
  ...
  const generatePDF = () => {
    ...
    autoTable(doc, {
      ...
      body: [
        ['Titolo di Accesso', `Voto: ${state.accessTitle.vote}${state.accessTitle.isLode ? ' + Lode' : ''}`, accessScore.toFixed(2)],
        
        // Detailed Cultural Titles
        ['Titoli Culturali', [
            state.culturalTitles.dottorato ? '- Dottorato di Ricerca' : '',
            state.culturalTitles.asn ? '- Abilitazione Scientifica Nazionale' : '',
            state.culturalTitles.specializzazione_sostegno_extra.length > 0 ? `- Specializzazioni Sostegno: ${state.culturalTitles.specializzazione_sostegno_extra.join(', ')}` : '',
            state.culturalTitles.hasAbilitazione ? `- Altre Abilitazioni: ${state.culturalTitles.abilitazioni.length}` : '',
            state.culturalTitles.hasConcorso ? `- Concorsi Ordinari: ${state.culturalTitles.concorsi.length}` : '',
            state.culturalTitles.hasMaster ? `- Master: ${state.culturalTitles.master_count}` : '',
            state.culturalTitles.hasPerfezionamento ? `- Perfezionamento: ${state.culturalTitles.perfezionamento_count}` : '',
            ...
            state.culturalTitles.pubblicazioni > 0 ? `- Pubblicazioni: ${state.culturalTitles.pubblicazioni}` : ''
        ].filter(Boolean).join('\n'), culturalScore.toFixed(2)],

        // Detailed Service
        ['Servizio', 
         state.service.map(s => {
             if (s.year) return `- Anno ${s.year}/${s.year+1} (${s.isSpecific ? 'Specifico' : 'Aspecifico'})`;
             return `- Periodo ${s.startDate} - ${s.endDate} (${s.isSpecific ? 'Specifico' : 'Aspecifico'})`;
         }).join('\n'), 
    ...
    doc.text('Questo calcolo è indicativo e non ha valore legale. Fare riferimento alle tabelle ufficiali del Ministero.', 14, doc.lastAutoTable.finalY + 10);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex justify-center">
        <button 
          onClick={generatePDF}
          className="bg-[#e31b23] text-white px-8 py-4 rounded-xl font-black text-sm uppercase tracking-widest hover:bg-black transition-colors shadow-lg flex items-center gap-2"
        >
